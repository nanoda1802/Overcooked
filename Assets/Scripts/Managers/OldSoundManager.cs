...
public class OldSoundManager : MonoBehaviour, IPool<AudioSource>
{
    ...
    private IEnumerator CoPlaySfx(AudioSource sfx)
    {
        sfx.Play();
        _activeSfx.Add(sfx);
        Debug.Log($"play [{sfx.name}({sfx.clip.name})] at {Time.time}");
        yield return new WaitForSeconds(sfx.clip.length); // [임시]
        Debug.Log($"wait for [{sfx.name}({sfx.clip.name})] by {Time.time}");
        while (sfx.isPlaying) // 혹시 모를 잔여 재생 대기
        {
            Debug.Log($"additional waiting for [{sfx.name}({sfx.clip.name})]");
            yield return null;
        }

        _activeSfx.Remove(sfx);
        ReturnToPool(sfx);
        Debug.Log($"return [{sfx.name}({sfx.clip?.name})] at {Time.time}");
    }
    ...
    public AudioSource PlayLoopingSfx(AudioClip clip)
    {
        ...
        _activeSfx.Add(sfx);
        sfx.Play();
        Debug.Log($"play looping {sfx.name}({sfx.clip.name})");
        return sfx;
    }
    ...
    private IEnumerator CoFadeOutLoopingSfx(AudioSource sfx)
    {
        ...
        Debug.Log($"fade out looping {sfx.name}({sfx.clip?.name})");
        sfx.Stop();
        _activeSfx.Remove(sfx);
        ReturnToPool(sfx);
    }
